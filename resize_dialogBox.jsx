...
function main()
{
    ...
    var newWidth = InputText.add ("edittext", [0, 0, 50, 20], "1");
    var newHeight = InputText.add ("edittext", [0, 0, 50, 20], "1");
    newHeight.visible = false;

    var SwitchPixelPercent = win2.add ('dropdownList', [20, 0, 50, 20], ["px","%"]);
    SwitchPixelPercent.selection = 0;
            ...
            if(file instanceof File && file.hidden == false)
            {
                if(file.name.match(/\.(psd|png|jpg|jpeg|gif)$/))
                    ...
                    if(ScaleDimension.length > 1)
                    {
                        ...
                        app.activeDocument.resizeImage(docWidth * ScaleDimension[0]);
                        app.activeDocument.close(SaveOptions.SAVECHANGES);
                        }
                    }
                else
